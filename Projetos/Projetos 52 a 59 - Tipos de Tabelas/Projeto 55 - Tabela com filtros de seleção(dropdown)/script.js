function atualizarTotais(contadorLinhas, somaSalarios) {
    ...
    document.getElementById('somaSalario').textContent = somaSalarios.toLocaleString('pt-BR', {style: 'currency', currency: 'BRL'});
...
function exportarParaExcel() {
    ...
    var tabelaClone = tabelaOriginal.cloneNode(true); //clona a tabelaOriginal
    ...
    //Usa a tabela clonada para exportação.
    var workbook = XLSX.utils.table_to_book(tabelaClone, {

        sheet: "Funcionarios",
        display: true //Opção para manter que a formatação visual da pagina seja mantida no EXCEL.

    });
    //Utiliza a biblioteca XLSX para converter a tabela(sem selects) em um workbook do excel.


    var dataAtual = new Date();

    var nomeArquivo = 'funcionarios_' + dataAtual.toLocaleDateString('pt-BR').replace(/\//g, '-') + '.xlsx';
    //Cria uma strinf para o nome do arquivo, com a data atual substituindo barras por hífens↑↑↑↑

    XLSX.writeFile(workbook, nomeArquivo);
    //Escreve o workbook criado em um arquivo Excel físico com o nome definido, que é então
    //  automaticamente baixado pelo navegador.
